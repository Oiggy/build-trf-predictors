#!/usr/bin/env python3
"""
gammatone_fbins_onsets_overt.py

Create pair-specific overt onset predictors for two-speaker cocktail-party
conditions.
...
Equation implemented here:
    overt = min(source_onset, mixture_onset)
...
Typical use from the project root:
    python gammatone_fbins_onsets_overt.py --conditions diotic binaural dichotic --overwrite

Safer first run:
    python gammatone_fbins_onsets_overt.py --conditions diotic --dry-run
"""
...
MODE = "overt"  # "masked" or "overt"
SCRIPT_PREFIX = "gammatone_fbins_onsets_overt"
...
def generate(
    ...
    with h5py.File(input_path, "r") as input_h5, h5py.File(output_path, "w") as output_h5:
        ...
        output_h5.attrs["equation"] = "overt=min(source,mixture)"
