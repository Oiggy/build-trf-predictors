...
onsets.h5 (8-bin features added):
    "<stimulus_name>/freq_bins/100-310"    # Example bin
    "<stimulus_name>/freq_bins/328-684"    # Example bin
...
from params import RESAMPLING_FREQUENCY
...
def ensure_fbins_envelopes():
    ...
    if not ENVELOPES_H5.exists():
        ...
        script_path = SCRIPT_DIR / "gammatone_fbins_envelopes.py"
...
def main():
            ...
            with h5py.File(ONSETS_H5, "r", locking=False) as h5:
                ...
                if first_stem in h5:
                    if "freq_bins" in h5[first_stem]:
                        freq_bins_group = h5[first_stem]["freq_bins"]
                        if any(label in freq_bins_group for label in bin_labels):
    ...
    with h5py.File(ENVELOPES_H5, "r", locking=False) as h5_env, \
         h5py.File(ONSETS_H5, "a", locking=False) as h5_onset:
