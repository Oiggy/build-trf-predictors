#!/usr/bin/env python3
"""
gammatone_fbins_onsets_masked.py - Compute 8-band masked onsets

Follows Brodbeck et al. (2020, PLOS Biology):

    O_masked = max(O_source - O_mixture, 0)

Masked onsets are the portion of a source speaker's onsets that are
hidden by the competing speaker in the mixture. They represent acoustic
features present in the isolated source but NOT visible in the mixture.
...
USAGE
====================================================================
    python gammatone_fbins_onsets_masked.py
"""
...
logger = setup_logging(
    ...
    output_dir=OUT_DIR,
    script_name="gammatone_fbins_onsets_masked"
)
...
def main():
    logger.info("=" * 70)
    logger.info("gammatone_fbins_onsets_masked.py")
    logger.info("  O_masked = max(O_source - O_mixture, 0)")
    logger.info("=" * 70)
    ...
    logger.info(f"Output:               {ONSETS_MASKED_H5}")
    logger.info("")
    ...
    with h5py.File(ONSETS_H5, "r", locking=False) as h5_in, \
         h5py.File(ONSETS_MASKED_H5, "w") as h5_out:
        ...
        for idx, (source, mixture) in enumerate(SOURCE_MIXTURE_PAIRS, 1):
            ...
            for bin_label in bin_labels:
                ...
                # O_masked = max(O_source - O_mixture, 0)
                o_masked = np.maximum(o_source - o_mixture, 0.0)

                out_key = f"{source}/freq_bins/{bin_label}"
                write_dataset(h5_out, out_key, o_masked.reshape(-1, 1), {
                    "sfreq": RESAMPLING_FREQUENCY,
                    "source": source,
                    "mixture": mixture,
                    "formula": "max(O_source - O_mixture, 0)",
                    "bin_label": bin_label,
                })
    ...
    out_png = OUT_DIR / "grid_fbins_onsets_masked.png"
    ...
    if len(processed) > 0:
        ...
        fig.suptitle(f"Masked Onset Spectrograms ({VIZ_DURATION_S:.0f} second)", fontsize=11, fontweight="bold")
