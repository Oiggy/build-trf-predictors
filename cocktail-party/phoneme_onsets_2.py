"""
Generate and Integrate Phoneme Onset Predictors into Existing Matrices

Overview
--------
This script adds a new predictor family, **phoneme onset**, to your TRF/mTRF
predictor matrices. It creates human-readable, space-style predictor columns
(e.g., "phoneme onset dichotic attend 2"), substitutes referenced WAV filenames
with Eelbrain NDVar objects loaded from per-stimulus *_phoneme_onsets.pickle
files, zero-fills any gaps to preserve shapes/dims, and merges the results into
the existing matrices.

Processing steps
----------------
1) Load stimulus matrix
   • Reads matrix_stimuli.csv, extracts condition names from headers that end in
     " 1"/" 2" or "_1"/"_2" (both styles supported).

2) Discover stimulus columns and WAVs
   • Resolves the actual columns present for stimulus 1 and 2.
   • Collects unique .wav filenames referenced in those columns.

3) Build an empty predictor matrix
   • For each non-excluded header, creates new columns prefixed with
     "phoneme onset " (space style, no underscores).

4) Load per-stimulus NDVars
   • Scans derivatives/predictors/phoneme_onsets for files named
     "<basename>_phoneme_onsets.pickle".
   • Unpickles the "phoneme_onsets" NDVar and standardizes NDVar.name to "<wav>.wav".
   • Builds a mapping: { "<wav>.wav": { "phoneme onset": NDVar } }.
   • If strict_match=True, only includes entries whose WAV exists in the stimulus folder.

5) Populate the predictor matrix
   • Replaces each WAV filename cell with the corresponding NDVar by matching the
     column prefix ("phoneme onset").
   • Fills missing/empty cells with zero NDVars matched in dims/shape (robust filling).

6) Split, merge, and save
   • Splits new predictors into stimulus-1 and stimulus-2 views (accepts both " 1"/"_1").
   • Merges each view into:
       matrix_predictors_1.pkl and matrix_predictors_2.pkl
     (overwrites existing columns with the same name; adds new ones otherwise).
   • Saves the merged DataFrames.
...
• Stimulus WAVs in the configured stimulus directory
• Phoneme onset pickles:
  binaural-cocktail-bids/derivatives/predictors/phoneme_onsets/*_phoneme_onsets.pickle
...
• Updated predictor_names.txt (script directory)

Conventions & assumptions
-------------------------
• Predictor columns use human-readable space style:
  "phoneme onset <condition> <stimulus_index>".
• Header parsing accepts both " … 1/2" and " …_1/_2".
• Zero-fill ensures consistent NDVar shapes/dims across rows/columns.

Customize here
--------------
• `new_predictor_names` to add more predictors.
• Directory paths/patterns in `build_phoneme_onsets_map`.
...
def build_phoneme_onsets_map(stimulus_dir, phoneme_onsets_dir,strict_match=True):
    """
    Returns a dict: mapping[wav_name] = {'phoneme': NDVar}
    ...
    wo_dir = Path(phoneme_onsets_dir)

    wav_names = {p.name for p in stim_dir.glob("*.wav")}
    mapping = {}

    suffix = "_phoneme_onsets.pickle"
    for pkl in wo_dir.glob(f"*{suffix}"):
        ...
        if "phoneme_onsets" not in df.columns:
            raise RuntimeError(f"'phoneme_onsets' column missing in {pkl}")

        ndvar = df["phoneme_onsets"].iloc[0]  # NDVar expected
        ...
        mapping[wav_name] = {"phoneme onset": ndvar}
...
new_predictor_names = ['phoneme onset']
...
stim_ndvar_dict = build_phoneme_onsets_map(
    stimulus_dir="binaural-cocktail-bids/stimulus",
    phoneme_onsets_dir="binaural-cocktail-bids/derivatives/predictors/phoneme_onsets",
    strict_match=True)
