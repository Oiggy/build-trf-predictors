"""
gammatone_fbins_masked_onsets.py - Compute 8-band masked onsets and save to onsets_masked.h5

====================================================================
WHAT THIS SCRIPT DOES
====================================================================
Computes masked onset predictors per frequency bin using the formula:

    Omasked = max(Osource - Omixture, 0)
...
Masked onsets capture the degree to which a source onset is NOT suppressed
by the competing acoustic mixture. When the source onset exceeds the mixture
onset at a given time sample, the difference is preserved; otherwise it is
zeroed out.
...
therefore excluded (masking does not apply when a speaker is heard alone).
...
Key structure is identical to onsets.h5, so onsets_masked.h5 can be used
...
USAGE
====================================================================
python gammatone_fbins_masked_onsets.py
...
ONSETS_MASKED_H5 = OUT_DIR / "onsets_masked.h5"

# Build unique (source, mixture) pairs from both trigger lists
# Each source stimulus is always paired with the same mixture stimulus
# across all experimental conditions — zip preserves that pairing per trial.
_all_pairs = (
...
logger = setup_logging(
    ...
    output_dir=OUT_DIR,
    script_name="gammatone_fbins_masked_onsets"
)
...
def masked_onset_from_onsets(source_onset, mix_onset):
    """
    Compute masked onset using:
        Omasked = max(Osource - Omixture, 0)
    ...
    Returns
    -------
    masked : 1D np.ndarray
        Masked onset (same length as inputs).
    """
    ...
    return np.maximum(source - mix, 0.0)
...
def plot_masked_onset_spectrograms(h5_path, source_names, bin_labels, out_png):
    """
    Generate a spectrogram grid showing 1 second of masked onsets
    for each source stimulus across all frequency bins.
    """
    logger.info("  Generating masked onset spectrogram grid...")

    # Build frequency edges for the y-axis from bin labels
    freq_edges = [F_MIN]
    ...
    fig.suptitle(
        "Gammatone Masked Onset Spectrograms — Omasked = max(Osource − Omixture, 0) — 1 second",
        fontsize=12
    )
...
def main():
    logger.info("")
    logger.info("=" * 70)
    logger.info("STARTING: gammatone_fbins_masked_onsets.py")
    logger.info("=" * 70)
    logger.info("Formula: Omasked = max(Osource - Omixture, 0)")
    ...
    out_png = OUT_DIR / "grid_8bin_masked_onsets.png"

    # ── Step 3: Check if already complete ────────────────────────────────
    if ONSETS_MASKED_H5.exists() and out_png.exists():
        try:
            with h5py.File(ONSETS_MASKED_H5, "r", locking=False) as h5:
                all_done = all(
                    any(f"{src}/freq_bins/{label}" in h5 for label in bin_labels)
                    for src, _ in SOURCE_MIXTURE_PAIRS
                )
            if all_done:
                logger.info("Masked onsets already exist in onsets_masked.h5")
    ...
    # ── Step 4: Compute and save masked onsets ────────────────────────────
    logger.info("Processing Parameters:")
    logger.info(f"  Sampling rate: {RESAMPLING_FREQUENCY} Hz")
    logger.info(f"  N bins:        {N_BINS}")
    logger.info(f"  Formula:       Omasked = max(Osource - Omixture, 0)")
    logger.info(f"  Output file:   {ONSETS_MASKED_H5}")
    logger.info("")

    source_names_processed = []

    with h5py.File(ONSETS_H5, "r", locking=False) as h5_src, \
         h5py.File(ONSETS_MASKED_H5, "a", locking=False) as h5_out:
        ...
        for pair_idx, (source_name, mix_name) in enumerate(SOURCE_MIXTURE_PAIRS, 1):
            ...
            for bin_idx, bin_label in enumerate(bin_labels, 1):
                ...
                try:
                    masked = masked_onset_from_onsets(source_onset, mix_onset)
                except ValueError as e:
                    logger.warning(f"    {e} — skipping bin")
                    continue

                n_nonzero     = int(np.sum(masked > 0))
                total_source  = int(np.sum(source_onset > 0))
                pct_surviving = (n_nonzero / total_source * 100) if total_source > 0 else 0.0

                logger.info(f"    masked onset shape:      {masked.shape}")
                logger.info(f"    source onsets (nonzero): {total_source}")
                logger.info(f"    masked onsets (nonzero): {n_nonzero}  "
                            f"({pct_surviving:.1f}% of source surviving masking)")

                out_key = f"{source_name}/freq_bins/{bin_label}"
                attrs   = {
                    "sfreq":            int(RESAMPLING_FREQUENCY),
                    "band_label":       bin_label,
                    "onset_definition": "masked onset: max(Osource - Omixture, 0)",
                    "source_stim":      source_name,
                    "mixture_stim":     mix_name,
                    "binning":          "ERB-spaced",
                }
                write_dataset(h5_out, out_key, masked.reshape(-1, 1), attrs)
    ...
    logger.info(f"Masked onsets saved to {ONSETS_MASKED_H5}")
    logger.info(f"Sources written: {source_names_processed}")
    logger.info("")

    # ── Step 5: Visualization ─────────────────────────────────────────────
    logger.info("Generating visualization...")
    plot_masked_onset_spectrograms(ONSETS_MASKED_H5, source_names_processed, bin_labels, out_png)

    logger.info("")
    logger.info("=" * 70)
    logger.info("COMPLETED: gammatone_fbins_masked_onsets.py")
    logger.info("=" * 70)
