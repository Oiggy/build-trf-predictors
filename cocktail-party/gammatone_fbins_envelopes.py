...
~/Github Repositories/trf-cocktail-party/
...
from params import RESAMPLING_FREQUENCY, BANDPASS_FILTER_PARAMS
...
def ensure_spectrograms():
    ...
    if not SPECTRO_H5.exists():
        ...
        script_path = SCRIPT_DIR / "gammatone_compute_spectrograms.py"
...
def compute_8bin_envelopes(spectrogram, audio_sr):
    ...
    cf = centre_freqs(float(audio_sr), N_BANDS, float(F_MIN))

    erb_values = hz_to_erb(cf)
...
def compute_8bin_labels(audio_sr):
    ...
    for i in range(N_BINS):
        ...
        band_indices = np.where(in_bin)[0]
        if len(band_indices) > 0:
            low_freq  = int(round(float(cf[band_indices].min())))
            high_freq = int(round(float(cf[band_indices].max())))
...
def bandpass_1d(signal, sfreq, l_freq, h_freq):
    ...
    filtered = mne.filter.filter_data(
        signal, sfreq=float(sfreq), l_freq=float(l_freq),
        h_freq=float(h_freq), verbose=False
    )
...
def resample_1d(signal, sfreq_in, sfreq_out):
    ...
    resampled   = mne.filter.resample(
        signal, up=up_factor, down=down_factor,
        npad="auto", axis=0, verbose=False
    )
...
def main():
    ...
    with h5py.File(SPECTRO_H5, "r", locking=False) as h5_spec, \
         h5py.File(ENVELOPES_H5, "a", locking=False) as h5_env:
